...
if residual:
    fig, (ax, ax1) = plt.subplots(2, sharex='all', gridspec_kw={'height_ratios': [6, 1.5]}, figsize=(7,7))
else:
    fig, ax = plt.subplots()
...
for i in range(1,len(instrument_data)):
    if instrument_data[i] != list_intruments[-1]:
        ax.errorbar(v_data_inst, vFv_data_inst, yerr=(err_data_inst_down, err_data_inst_up), label=str(list_intruments[-1]), 
                    markersize=3, elinewidth=1, color = cmap(len(list_intruments)), fmt=filled_markers[len(list_intruments)-1])
...
if residual:
    ...
    if residual == "sigma":
        ...
        sign = diff > 0
        vFv_sigma = sign*(vFv_data-f_best(v_data))/err_data[0] - ~sign*(f_best(v_data)-vFv_data)/err_data[1]
