...
def run_mcmc(configs, param_min_vals=None, param_max_vals=None, backend_file=RESULTS_FOLDER + "/backend.h5", p0=None, p0_file=None, eic_p0_from_std=False, min_freq=None, max_freq=None, prev_files=False, use_param_file=True, eic=False):
    ...
    bjet_command_1, bjet_command_2 = blazar_model.command_line_sub_strings(min_freq=min_freq, max_freq=max_freq,
    ...
    if eic_p0_from_std:
        ...
        p0 = blazar_utils.random_eic_from_std(p0, configs["n_walkers"], param_min_vals, param_max_vals,
        ...
        if len(p0.coords) != configs["n_walkers"]:
            raise ValueError("p0 start has the incorrect number of walkers. p0 has " + str(len(p0.coords)) + " but the number of walkers is " + configs["n_walkers"] + ".")
        if configs["eic"] and len(p0.coords[0]) != 13:
            raise ValueError("for an EIC run, p0 coordinates must have 13 values. " + str(len(p0.coords[0])) + " values present.")
        elif not configs["eic"] and len(p0.coords[0]) != 9:
            raise ValueError("for a non-EIC run, p0 coordinates must have 9 values. " + str(len(p0.coords[0])) + " values present.")
        print("p0 from file")
    elif p0 is not None:
        if len(p0) != configs["n_walkers"]:
            raise ValueError("p0 start has the incorrect number of walkers. p0 has " + str(
                len(p0.coords)) + " but the number of walkers is " + configs["n_walkers"] + ".")
        if configs["eic"] and len(p0[0]) != 13:
            raise ValueError(
                "for an EIC run, p0 coordinates must have 13 values. " + str(len(p0[0])) + " values present.")
        elif not configs["eic"] and len(p0[0]) != 9:
            raise ValueError(
                "for a non-EIC run, p0 coordinates must have 9 values. " + str(len(p0[0])) + " values present.")
    ...
    sampler = emcee.EnsembleSampler(configs["n_walkers"], modelProperties(eic, fixed_params=configs["fixed_params"]).NUM_DIM,
                                    ...
                                    kwargs={'param_min_vals': param_min_vals,
                                            ...
                                            'tau_var': configs["tau_variability"],
                                            'use_variability': configs["use_variability"],
                                            'redshift': configs["redshift"], 'min_freq': min_freq, 'max_freq': max_freq,
...
def mcmc(config_file=None, directory=None, folder_label=None, p0=None, p0_label=None, p0_file=None, eic_p0_from_std=False, 
    ...
    results = run_mcmc(configs, param_min_vals=param_min_vals, param_max_vals=param_max_vals, backend_file=backend,
                       p0=p0, p0_file=p0_file, eic_p0_from_std=eic_p0_from_std, prev_files=prev_files, use_param_file=use_param_file, eic=configs["eic"])
    ...
    blazar_report.save_plots_and_info(configs, data, param_min_vals, param_max_vals, folder=directory, sampler=sampler,
                                      use_sampler=True, description=description, time=results[1], p0_source=p0_label,
                                      redshift=configs["redshift"], eic=configs["eic"])
