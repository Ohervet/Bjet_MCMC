...
//******************************************************************
  ...
  std::cout<<"read data...."<<std::endl;
   ...
   std::cout<<"interpolate"<<std::endl;
    ...
    //k13 = interpol(k12,nb10,k12,nb10,tot11,fflux10,freqtot11,freq10,tot12,freqtot12); //host gal
...
// Chi2
   // reduced Chi2 of the fit on XRT, BAT, Fermi and VERITAS
   double Chi2_XRT = 0.;
   Double_t ymodel_XRT[10];
   double Chi2_BAT = 0.;
   Double_t ymodel_BAT[7];
   double Chi2_Fermi = 0.;
   Double_t ymodel_Fermi[5];
   double Chi2_VERITAS = 0.;
   Double_t ymodel_VERITAS[6];
   for (i=0;i<(500);i++){
        for(j=0;j<nx;j++){
        ...
        for(j=0;j<5;j++){
           if((freqtot11b[i]<=E_F[j]) && (freqtot11b[i+1]>E_F[j])){
               ...
               if (F_F[j] >= ymodel_Fermi[j]){
                  Chi2_Fermi += pow((F_F[j]-ymodel_Fermi[j]) / deltaFmin_F[j],2);
               }else{
                  Chi2_Fermi += pow((F_F[j]-ymodel_Fermi[j]) / deltaFmin_F[j],2);
               }
           }
        ...
        for(j=0;j<7;j++){
           if((freqtot11b[i]<=E_V[j]) && (freqtot11b[i+1]>E_V[j])){
               ...
               if (F_V[j] >= ymodel_VERITAS[j]){
                  Chi2_VERITAS += pow((F_V[j]-ymodel_VERITAS[j]) / deltaFmin_V[j],2);
               }else{
                  Chi2_VERITAS += pow((F_V[j]-ymodel_VERITAS[j]) / deltaFmax_V[j],2);
               }
           }
       }
   }
   std::cout<<"Chi2_XRT/DOF = "<<Chi2_XRT<<"/"<<nx<<std::endl;
   std::cout<<"Chi2_BAT/DOF = "<<Chi2_BAT<<"/"<<7<<std::endl;
   std::cout<<"Chi2_Fermi/DOF = "<<Chi2_Fermi<<"/"<<5<<std::endl;
   std::cout<<"Chi2_VERITAS/DOF = "<<Chi2_VERITAS<<"/"<<7<<std::endl;
...
// Make Plots
 ...
 // 1. Particle spectra
   ...
   // EIC from dust torus
    ...
    //interpolation
	  ...
	  //grtot = new TGraph(498,freqtot13,tot13);
 ...
 /*
    ...
    */
	...
	leg = new TLegend(0.15,0.73,0.92,0.85);
	leg->SetTextSize(0.03);
	leg->SetFillColor(kWhite);
	leg->SetNColumns(5);
	leg->SetMargin(0.2);
	leg->AddEntry(gr0_0,"VLBA","lep");
        leg->AddEntry(gr_caha,"CAHA","lep");
        leg->AddEntry(gr_xrt,"Swift-XRT","lep");
        leg->AddEntry(gr_FERMI,"Fermi-LAT","f");
        leg->AddEntry(gr_Vd,"VERITAS EBL deabsorbed","lep");
        leg->AddEntry(gr_wise,"WISE","lep");
        leg->AddEntry(gr_flwo,"FLWO 48''","lep");
        leg->AddEntry(gr_bat,"Swift-BAT","lep");
        leg->AddEntry(gr_VERITAS,"VERITAS","f");
