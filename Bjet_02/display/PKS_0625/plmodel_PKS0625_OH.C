...
//*******************************************
 ...
 void plmodel()
{
  ifstream file1,file1p,file2,file2p,file3,file3p,file4,file5,file6,file7,file7p,file8,file101,file9,file9p,file9a,file9ap,file10,file11,file11p,file12,file12p,file12a,file12b,file13,file13p,file14,file14p, file15, FILE1;
  char   name;
  char   stmp[256];
  const int nb=1500, ng = 1304, nda = 775,ndb = 129,n1 = nb*2, n3 = nb*3,n4 = nb*4,n5 = nb*5,n6 = nb*6,n7= nb*7,n2= n7+ng, nbFu = 59;
  int i,j,a,k1=0,k2=0,k3=0,k4=0,k5=0,k6=0,k7=0,k8=0,k9=0,k10=0,k11=0,k12=0,k13=0,k0=0,nb2=0,nb3=0,nb4=0,nb5=0,nb6=0,nb7=0,nb8=0,nb9=0,nb9a=0,nb10=0,nb11=0,nb12=0,nb12a=0,nb12b=0,nb13=0,nb14=0,nb15 = 0,lx,l,wehavedata;
  ...
  Double_t freq[nb],flux[nb],ffluxc[nb],fluxc[nb],fflux[nb],freq2[nb],flux2[nb],fflux2[nb],freq3[nb],flux3[nb],fflux3[nb],freq101[nb],fluxc101[nb],ffluxc101[nb],flux101[nb],fflux101[nb],freq9[nb],fluxc9[nb],ffluxc9[nb],flux9[nb],fflux9[nb],
	   ...
	   fflux12b[ndb], freq15[nbFu], fflux15[nbFu];
  ...
  Double_t freqti[n1],fluxti[n1],freqt[n2],fluxt[n2],ge[nb],ne[nb],gse[nb],nse[nb],gp[nb],np[nb],eqm[3],freqinter[n2],ffluxinter[n2],tot[n1],freqtot[n1],tot2[n3],freqtot2[n3],tot3[n4],freqtot3[n4],tot4[n5],freqtot4[n5],tot5[n6],freqtot5[n6],
           tot6[n7],freqtot6[n7],tot7[n7],freqtot7[n7],tot8[n7],freqtot8[n7],tot9[n7],freqtot9[n7],tot10[n7],freqtot10[n7],tot11[n7],freqtot11[n7],tot12[n2],freqtot12[n2];
  ...
  //Initialisation
      ...
      file10.open("data/PKS_0625-354/PKS_0625_gal.dat"); // host galaxy
      //file10.open("data/PKS_0625-354/host_gal_PKS_0625.dat"); // host galaxy
...
//******************************************************************
  ...
  std::cout<<"read data...."<<std::endl;
   ...
   std::cout<<"interpolate"<<std::endl;
    ...
    k6 = interpol(k5,nb11,n6,nb,tot4,fflux11,freqtot4,freq11,tot5,freqtot5);
    ...
    k13 = interpol(k12,nb10,n2,n2,tot11,fflux10,freqtot11,freq10,tot12,freqtot12); //host gal
    
    //k13 = interpol(k12,nb,n7,nb10,fflux10,tot11,freq10,freqtot11,tot12,freqtot12); //host gal
    
    

// Chi2
/*    
   //chi2 pondéré, 5 densités de données
   rho1 = (E1[8]-E1[0])/9; //planck
   rho2 = (E1[14]-E1[9])/6; //UVOT
   rho3 = (E1[22]-E1[15])/8; //XRT
   rho4 = (E1[25]-E1[23])/3; //Fermi
   rho5 = (E[n1a]-E[(n1a-8)])/8; //HESS
 
   rho_m = (rho1+rho2+rho3+rho4+rho5)/5;
   rho1 = rho_m/rho1;
   rho2 = rho_m/rho2;
   rho3 = rho_m/rho3;
   rho4 = rho_m/rho4;
   ...
   // Quasi-simultaneous data
   for(int i=0;i<32;i++){    
      for (int j=0;j<(n7-1);j++){
	if((freqtot6[j]<=E1[i]) && (freqtot6[j+1]>E1[i])){
	  ymodel[i] = (tot6[j]+(tot6[j+1]-tot6[j])*(E1[i]-freqtot6[j])/(freqtot6[j+1]-freqtot6[j]));
	}
      }
      if(ymodel[i] <= F1[i]){
	sigma[i] = deltaF1[i];//deltaF1[i]/F1[i];
	if(i <= 8){
	  sigma_pond[i] = deltaF1[i]/(F1[i]*rho1);
	  Chi2_planck = Chi2_planck + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	}
	else if(i <= 12){
	  sigma_pond[i] = deltaF1[i]/(F1[i]*rho2);
	  Chi2_wise = Chi2_wise + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	}
	else if(i <= 18){
	  sigma_pond[i] = deltaF1[i]/(F1[i]*rho2);
	  Chi2_uvot = Chi2_uvot + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	}
	else if(i <= 26){
	  sigma_pond[i] = deltaF1[i]/(F1[i]*rho3);
	  Chi2_xrt = Chi2_xrt + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	}
	else if((i > 27) && (i <= 30)){
	  sigma_pond[i] = deltaF1[i]/(F1[i]*rho4);
	  Chi2_fermi = Chi2_fermi + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	}
      }
      else{
	sigma[i] = deltaF1max[i];//deltaF1max[i]/F1[i];
	if(i <= 8){
	  sigma_pond[i] = deltaF1[i]/(F1[i]*rho1);
	  Chi2_planck = Chi2_planck + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	}
      ...
      if(deltaF1[i] != 0)){
        Chi2 = Chi2 + ((F1[i]-ymodel[i])/sigma[i])*((F1[i]-ymodel[i])/sigma[i]);
	Chi2_pond = Chi2_pond + ((F1[i]-ymodel[i])/sigma_pond[i])*((F1[i]-ymodel[i])/sigma_pond[i]);
      }
   }
   ...
   for(int i=0;i<8;i++){
      for (int j=0;j<(n7-1);j++){
	if((freqtot6[j]<=E[i+(n1a-9)]) && (freqtot6[j+1]>E[i+(n1a-9)])){
	  ymodel[i] = (tot6[j]+(tot6[j+1]-tot6[j])*(E[i+(n1a-9)]-freqtot6[j])/(freqtot6[j+1]-freqtot6[j]));
	}
      }
      if(ymodel[i] <= F[i+(n1a-9)]){
	sigma[i] = deltaF[i+(n1a-9)];//deltaF[i+(n1a-9)]/F[i+(n1a-9)];
	sigma_pond[i] = deltaF[i+(n1a-9)]/(F[i+(n1a-9)]*rho5);
	Chi2_hess = Chi2_hess + ((F[i+(n1a-9)]-ymodel[i])/sigma[i])*((F[i+(n1a-9)]-ymodel[i])/sigma[i]);
      }
      else{
	sigma[i] = deltaFmax[i+(n1a-9)];//deltaFmax[i+(n1a-9)]/F[i+(n1a-9)];
	sigma_pond[i] = deltaFmax[i+(n1a-9)]/(F[i+(n1a-9)]*rho5);
	Chi2_hess = Chi2_hess + ((F[i+(n1a-9)]-ymodel[i])/sigma[i])*((F[i+(n1a-9)]-ymodel[i])/sigma[i]);
      }
      if(deltaF[i+(n1a-9)] != 0)){
        Chi2 = Chi2 + ((F[i+(n1a-9)]-ymodel[i])/sigma[i])*((F[i+(n1a-9)]-ymodel[i])/sigma[i]);
	Chi2_pond = Chi2_pond + ((F[i+(n1a-9)]-ymodel[i])/sigma_pond[i])*((F[i+(n1a-9)]-ymodel[i])/sigma_pond[i]);
      }
   }
   std::cout<<"Chi2= "<<Chi2<<std::endl;
   std::cout<<"Chi2 pondere= "<<1.0<<std::endl;
   std::cout<<"Chi2 Planck= "<<Chi2_planck/9<<std::endl;
   std::cout<<"Chi2 WISE= "<<Chi2_wise/4<<std::endl;
   std::cout<<"Chi2 UVOT= "<<Chi2_uvot/6<<std::endl;
   std::cout<<"Chi2 XRT= "<<Chi2_xrt/8/10<<std::endl; //Chi2 modifié pour xrt, on permet une erreur plus grande (facteur 10)
   std::cout<<"Chi2 Fermi= "<<Chi2_fermi/3<<std::endl;
   std::cout<<"Chi2 HESS= "<<Chi2_hess/8<<std::endl;
   std::cout<<"Chi2 total= "<<Chi2_planck/9+Chi2_wise/4+Chi2_uvot/6+Chi2_xrt/8/10+Chi2_fermi/3+Chi2_hess/8<<std::endl;
   
   //std::cout<<"deltaF1= "<<deltaF1[1]<<std::endl;
   //std::cout<<"deltaF1max= "<<deltaF1max[1]<<std::endl;
...
// Make Plots
 ...
 // 1. Particle spectra
   ...
   // SSC
	...
	// SSC unabsorbed  (plot all points)
	gr2 = new TGraph(k0,freq,fflux); 
  ...
  /*  
    ...
    //interpolation
	  grtot = new TGraph(k13-1,freqtot12,tot12);
	  grtot->SetLineColor(kOrange+4);//kOrange+1
          grtot->SetLineStyle(1);
	  grtot->SetLineWidth(3);
          sed->Add(grtot);
    ...
    */
    ...
    xsed->SetMinimum(-11.6);
    xsed->SetMaximum(-10.5);
    xsed->Draw("AP");
    xsed->GetXaxis()->SetLimits(14.3,15.4);
    xsed->GetXaxis()->SetLabelSize(0.08);
    xsed->GetYaxis()->SetLabelSize(0.08);
    xsed->GetXaxis()->SetNdivisions(6);
    xsed->GetYaxis()->SetTitle("");
    xsed->GetXaxis()->SetTickLength(0.08);
    xsed->GetYaxis()->SetTickLength(0.08);
    xsed->Add(xgrdata,"P");
    xsed->Add(gr10);
    xsed->Draw("C");
    */
