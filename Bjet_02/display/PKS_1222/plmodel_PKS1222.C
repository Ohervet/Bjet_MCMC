...
//*******************************************
 ...
 void plmodel()
{
  ifstream file1,file1p,file2,file2p,file3,file3p,file4,file5,file6,file7,file7p,file8,file101,file9,file9p,file9a,file9ap,file10,file11,file11p,file12,file12p,file12a,file12b,file13,file13p,file14,file14p,fileModel1,fileModel2, FILE1;
  char   name;
  char   stmp[256];
  ...
  int nbM = 207;
  Double_t freqM[207],ffluxM[207];
  Double_t freqM2[207],ffluxM2[207];
  ...
  //Initialisation
      ...
      //fileModel1.open("data/1ES_1215+303/model_Flare2017.dat"); // previous full model
      //fileModel2.open("data/1ES_1215+303/model_Postflare2017.dat"); // previous full model
...
//******************************************************************
  ...
  std::cout<<"read data...."<<std::endl;
    ...
    k2 = interpol(nb7,nb2,n1,nb, logffluxext,fflux2,logfreqext,freq2,tot,freqtot); //Jet syn + blob syn
    Double_t totb[500], freqtotb[500];
    Quad_interpol(k2, tot, freqtot, 500, totb, freqtotb);
    k3 = interpol(498,k0,498,nb, totb,ffluxc,freqtotb,freq,tot2,freqtot2); //+ blob ssc
    //k3 = interpol(k2,k0,n3,nb, tot,ffluxc,freqtot,freq,tot2,freqtot2); //+ blob ssc
    ...
    k9 = interpol(498,nb13,498,nb,tot7b,logffluxjssc,freqtot7b,logfreqjssc,tot8,freqtot8); // ssc jet etendu
    ...
    Quad_interpol(k12, tot11, freqtot11, 500, tot11b, freqtot11b);
...
// Chi2
   ...
   //loop on data points
   for(int i=0;i<nn;i++){
      // loop on model points
      for (int j=0;j<(499);j++){
	if((freqtot11[j]<=E[i]) && (freqtot11[j+1]>E[i])){
        //linear interpolation
	  ymodel[i] = (tot11[j]+(tot11[j+1]-tot11[j])*(E[i]-freqtot11[j])/(freqtot11[j+1]-freqtot11[j]));
	}
      }
      //Consider asymetric error bars
      if(ymodel[i] <= F[i]){
	sigma[i] = deltaFmin[i];
      }else{
        sigma[i] = deltaFmax[i];
      }
      //don't take into account ULs
      if (sigma[i] != 0){
        aa = ((F[i]-ymodel[i])/sigma[i])*((F[i]-ymodel[i])/sigma[i]);
        Chi2 += aa;
        ii+=1;
        std::cout<<"Chi2[i]= "<<aa<<std::endl;
      }
   }
   std::cout<<"Chi2/dof= "<<Chi2<<"/"<<ii<<std::endl;
   ...
   /*
      if(!fileModel1){
         cout<<"Model file not found !"<<endl;     
            ...
            for (i = 0; i <= nbM; i++) {
            fileModel1>>freqM[i]>>ffluxM[i];

	    //        std::cout<<i<<" "<<freq[i]<<" "<<flux[i]<<" "<<fflux[i]<<" "<<fluxc[i]<<" "<<ffluxc[i]<<" "<<std::endl;

            if (!fileModel1.good()) break;
         }
       }   
      ...
      if(!fileModel2){
         cout<<"Model file not found !"<<endl;     
            ...
            for (i = 0; i <= nbM; i++) {
            fileModel2>>freqM2[i]>>ffluxM2[i];

	    //        std::cout<<i<<" "<<freq[i]<<" "<<flux[i]<<" "<<fflux[i]<<" "<<fluxc[i]<<" "<<ffluxc[i]<<" "<<std::endl;

            if (!fileModel2.good()) break;
         }
       }   
...
*/
    ...
    */
    ...
    if(nbM>0){
 	  pgrM = new TGraph(nbM,freqM,ffluxM);
	  pgrM->SetLineColor(1);
          pgrM->SetLineStyle(4);
	  pgrM->SetLineWidth(2);
          sed->Add(pgrM);   
    ...
    if(nbM>0){
 	  pgrM2 = new TGraph(nbM,freqM2,ffluxM2);
	  pgrM2->SetLineColor(1);
          pgrM2->SetLineStyle(4);
	  pgrM2->SetLineWidth(2);
          sed->Add(pgrM2);   
    }
 ...
 /*
    ...
    */
    ...
    sed->GetYaxis()->SetTitleSize(0.05);
    sed->GetYaxis()->SetTitleOffset(1.2);
    sed->GetYaxis()->SetLabelSize(0.05);
    /*
    ...
    */
    ...
    sed->Draw("C"); // to have the fit above the data points
 	...
 	leg = new TLegend(0.14,0.78,0.93,0.85);
    ...
    eVaxis->SetTitleOffset(1.1);
    eVaxis->SetTitleSize(0.05);
    eVaxis->SetTitleFont(42);
    eVaxis->SetLabelFont(42);
    eVaxis->SetLabelSize(0.05);
    eVaxis->Draw();
    ...
    Nuaxis->SetLabelSize(0.05);
    Nuaxis->SetTitleSize(0.05);
    Nuaxis->SetTitleOffset(1.0);
    ...
    /*
    TText *tt = new TText(15.5,-9.8,"2017 flare");
    tt->SetTextColor(14); 
    tt->SetTextSize(0.02);
    tt->Draw();
    */
