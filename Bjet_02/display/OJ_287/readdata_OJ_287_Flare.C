{
  ...
  int i, count, countUL, n0=55,n1=54,n2=32,nb0,nb1, gr0_0, nfs = 50, nVs = 900;
  Double_t A,B,E[n0],F[n0],C[n0],D[n0],deltaEmin[n0],deltaEmax[n0],deltaFmin[n0],deltaFmax[n0],,Euplim[n2],Fuplim[n2],dEminuplim[n2],dEmaxuplim[n2],dFuplim[n2];
  ...
  int nBU = 1;
  Double_t E_BU[nBU], F_BU[nBU],deltaEmin_BU[nBU],deltaEmax_BU[nBU],deltaFmin_BU[nBU],deltaFmax_BU[nBU];
  int nC = 1;
  ...
  //int n_F2 = 3;
  //Double_t E_F2[n_F2], F_F2[n_F2],deltaEmin_F2[n_F2],deltaEmax_F2[n_F2],deltaFmin_F2[n_F2],deltaFmax_F2[n_F2];
  const int n_V = 6;
  ...
  const double planck = 4.135667517e-15; // h in [eV s]
   ...
   file1.open("data/OJ_287/OJ287_SED_complementary_period2.dat");
   file0.open("data/OJ_287/OJ287_Flare_SED.dat");
   file_Fermi.open("data/OJ_287/Fermi_spectrum_Flare.dat");
   file_VERITAS.open("data/OJ_287/VERITAS_spectrum_flare.dat");
...
//******************************************************************
	...
	file_VERITAS>>Freq>>Flux>>DFluxLow>>DFluxHigh;
	    ...
	    DFmax_Vs[i] = log10((Flux+DFluxHigh)) - Flux_Vs[i];
            ...
            //std::cout<<i<<"\t"<<Freq_Vs[i]<<"\t"<<Flux_Vs[i]<<" \t"<<DFmin_Vs[i]<<"\t"<<DFmax_Vs[i]<<std::endl;
	...
	maxF=-9;
 	...
 	nb1=k;
	//display errorbars
	TGraphAsymmErrors *gr0 = new TGraphAsymmErrors(25,E,F,deltaEmin,deltaEmax,deltaFmin,deltaFmax);
	...
	// Host galaxy
	  ...
	  //gr10->Draw("C");
        ...
        //archival data
	gr1 = new TGraphAsymmErrors(n1-3,E1,F1,deltaEmin1,deltaEmax1,deltaFmin1,deltaFmax1);
	gr1->SetMarkerColor(15);
	...
	for(i=0;i<nBU;i++){
	  E_BU[i] = E[count+i];
	  F_BU[i] = F[count+i];
	  deltaEmin_BU[i] = deltaEmin[count+i];
	  deltaEmax_BU[i] = deltaEmax[count+i];
	  deltaFmin_BU[i] = deltaFmin[count+i];
	  deltaFmax_BU[i] = deltaFmax[count+i];
	  }
        gr_BU = new TGraphAsymmErrors(nBU,E_BU,F_BU,deltaEmin_BU,deltaEmax_BU,deltaFmin_BU,deltaFmax_BU);
	gr_BU->SetMarkerColor(1);
	gr_BU->SetLineColor(1);
	gr_BU->SetMarkerStyle(21);
	gr_BU->SetMarkerColor(1);
	gr_BU->Draw("P");
	...
	for(i=0;i<n_F2;i++){
	  E_F2[i] = E[count+i];
	  F_F2[i] = F[count+i];
	  deltaEmin_F2[i] = deltaEmin[count+i];
	  deltaEmax_F2[i] = deltaEmax[count+i];
	  deltaFmin_F2[i] = deltaFmin[count+i];
	  deltaFmax_F2[i] = deltaFmax[count+i];
	}
	gr_F2 = new TGraphAsymmErrors(n_F2,E_F2,F_F2,deltaEmin_F2,deltaEmax_F2,deltaFmin_F2,deltaFmax_F2);
	gr_F2->SetMarkerStyle(20);
        gr_F2->SetMarkerColor(15);
...
/*
	...
	leg->SetMargin(0.15);
        ...
        leg->AddEntry(gr1,"Archival data","lep");
	...
	leg->AddEntry(gr_V,"VERITAS","lep");
        ...
        leg->AddEntry(gr_BU,"43GHz-Core","lep");
