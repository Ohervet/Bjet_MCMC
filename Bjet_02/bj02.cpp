...
double int_spec_b(int a){
   ...
   if (N1 == a and N2 != a){
      return K1*((log(GAMMA_BRK)-log(GAMMA_MIN)) + pow(GAMMA_BRK,(N2-N1))/(a-N2)*(pow(GAMMA_MAX,(a-N2))-pow(GAMMA_BRK,a-N2)));
   }
   if (N1 != a and N2 == a){
      return K1*(1/(a-N1)*(pow(GAMMA_BRK,a-N1)-pow(GAMMA_MIN,a-N1)) + pow(GAMMA_BRK,N2-N1)*(log(GAMMA_MAX)-log(GAMMA_BRK)));
   }
   else{           
      return K1*(1/(a-N1)*(pow(GAMMA_BRK,a-N1)-pow(GAMMA_MIN,a-N1)) + pow(GAMMA_BRK,(N2-N1))/(a-N2)*(pow(GAMMA_MAX,a-N2)-pow(GAMMA_BRK,a-N2)));
   }
}
...
int main(int argc, char** argv) {
   ...
   cout << "SSC MODEL FOR SPHERICAL HOMOGENEOUS BLOB" << endl
	<< "EXTERNAL INVERSE COMPTON ON THE NUCLEUS RADIATION FIELD (HOT CORONA / ACCRETION DISK / DUST TORUS)" << endl
  ...
  sprintf(stmp1, "./data/%s_tor.dat", prefix);
   ...
   //if (CASE_EIC){
    ...
    if (CASE_JET == 0){
        sprintf(stmp, "./data/%s_ecs_disk.dat", prefix);
        sprintf(stmp1, "./data/%s_prev_ecs_disk.dat", prefix);
        if (ifexist(stmp)) {
            sprintf(comma, "mv ./data/%s_ecs_disk.dat ./data/%s_prev_ecs_disk.dat", prefix, prefix);
            if(system(comma)) return 1;
        } else if (ifexist(stmp1)){
            sprintf(comma, "rm ./data/%s_prev_ecs_disk.dat", prefix);
            if(system(comma)) return 1;
        }
    }
    ...
    for (i = 1; i <= NU_DIM; i++) {
        ...
        */
        ...
        for (j = 1; j <= 200; j++) {
            ...
            I_rad_ext_D[j] = L_BB_nuc[i] * LOR_B / (4.0 * M_PI * tmp_cur*tmp_cur) * tau * pow(tmp_cur/R_blr,2) / (1. + pow(tmp_cur/R_blr,4)); //Nalewajko 14
            tmp_cur += tmp_stp;
        }
   ...
   Ub_eicj = 4*M_PI/c*Simpson(I_eic_jet, NU, NU_DIM, 1, NU_DIM);
    ...
    for (i = 1; i <= NU_DIM; i++) {
        ...
        if (dtmp > 0.){
            ...
            kk = gg_abs(NU[i+1], I_rad_ext_Int, NU_DIM,
                        FreqTransO2S(NU_STR, DOP_B, z),
                        FreqTransO2S(NU_END, DOP_B, z), COM_PREC1, COM_PREC2);
    ...
    for (i = 1; i <= NU_DIM; i++) {
        ...
        // absorption by pair production on BLR soft photons, use of the already integrated rad field: I_rad_ext_Int
        kk = gg_abs(NU[i]*LOR_B, I_rad_ext_Int, NU_DIM,
                        FreqTransO2S(NU_STR, DOP_B, z)*LOR_B,
                        FreqTransO2S(NU_END, DOP_B, z)*LOR_B, COM_PREC1, COM_PREC2);
